...
NoiseGenerator::NoiseGenerator(float sampleRate)
    : sr_(sampleRate), frequency_(440.0f), // Add this initialization
      fadeTime_(200.0f), level_(0.5f), fadeCounter_(0.0f), colorType_(1),
      lpCoeff_(1.0f), hpCoeff_(0.0f), lpState_(0.0f), hpState_(0.0f),
      pink_b0_(0.0f), pink_b1_(0.0f), pink_b2_(0.0f), pink_b3_(0.0f),
      pink_b4_(0.0f), pink_b5_(0.0f), pink_b6_(0.0f), brown_last_(0.0f),
      seed_(1234567) {
  lowpass(6000.0f);
  highpass(150.0f);
}
...
void NoiseGenerator::color(int type) { colorType_ = std::clamp(type, 0, 2); }
...
float NoiseGenerator::generateWhite() { return random(); }
...
float NoiseGenerator::generatePink() {
  ...
  float pink = pink_b0_ + pink_b1_ + pink_b2_ + pink_b3_ + pink_b4_ + pink_b5_ +
               pink_b6_ + white * 0.5362f;
